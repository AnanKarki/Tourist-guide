...
<button id="myBtn" name="Write a Review">Write a review</button>
   ...
   <input class ="r" type="radio" id="star-5" name ="rating" value ="5" >
    ...
    <label for ="star-1" title ="1star"><i class="fa fa-star" hidden ="true"></i></label>
       ...
       @foreach($reviews as $post)
          ...
          <?php $place=$post['place'] ?> 
          <?php $user=$post['username'] ?>
          <?php $rate=$post['rating'] ?>
          <?php $comment=$post['comment'] ?>
          <?php $users =Auth::user()->email ?>
          <?php $value = DB::table('reviews')->where([
                         ['username', $user],
                         ['place', $place ],
                          ])->value('place')?>
          <?php $username = DB::table('reviews')->where([
                         ['username', $user],
                         ['place', $place ],
                          ])->value('username')?>
          ...
          @if($place === $value AND $username === $users)
         <?php $id = $post['id'] ?>

  <form method="post" action="{{action('ReviewController@update')}}" enctype="multipart/form-data">
     {{csrf_field()}}
      
         
              <input type="hidden" id="number" name="rating" value="{{$rate}}">
     <label >Comment</label>
    <textarea placeholder="Your View" id="area" name="comment" rows="8" cols="80" required>{{$comment}}</textarea>
    </div> 
    ...
    <label >Your Previous Rating</label>
         @if($post['rating'] === 5)
         <td>
              <div class="star-rating" id ="rates">
